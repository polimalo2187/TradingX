...
from app.database import (
    create_user, get_user, user_is_ready, get_user_stats,
    save_api_keys, save_user_capital, activate_trading, deactivate_trading
)
from app.trading_engine import trading_cycle

# Control para evitar múltiples hilos por usuario
active_threads = {}
...
async def activate_trading_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "El bot comenzará a analizar el mercado y ejecutar operaciones.",
        parse_mode="Markdown",
        reply_markup=main_menu
    )

    # Evitar múltiples hilos para el mismo usuario
    if user_id in active_threads and active_threads[user_id].is_alive():
        print(f"⚠️ Usuario {user_id} ya tiene un hilo activo.")
        return

    # Crear nuevo hilo
    thread = threading.Thread(target=trading_cycle, args=(user_id,), daemon=True)
...
async def deactivate_trading_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "El bot ya no ejecutará operaciones.",
        parse_mode="Markdown",
        reply_markup=main_menu
    )

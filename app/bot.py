...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "activate_trading":
        ...
        if balance < user.get("capital", 0):
            await query.edit_message_text(
                ...
                f"Capital configurado: {user.get('capital')} USDT\n"
                f"Balance real disponible: {balance} USDT",
                reply_markup=get_back_button()
            )
            return
        ...
        # Inicia trading sin bloquear el bot
    ...
    if data == "status":
        await query.edit_message_text(
            ...
            f"💰 Capital asignado: {user.get('capital', 0)} USDT",
            parse_mode="Markdown",
            reply_markup=get_back_button()
        )
        return
...
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" in text:
        try:
            ...
            save_api_keys(user_id, api.strip(), secret.strip())

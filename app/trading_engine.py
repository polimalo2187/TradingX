...
def calculate_quantity(usdt_amount, price):
    """
    Calcula la cantidad de tokens según el capital del usuario.
    """
    if price <= 0:
        return 0
...
def open_trade(user_id, symbol, trade_plan):
    """
    Ejecuta una compra MARKET real en CoinW.
    """

    capital = get_user_capital(user_id)

    # Validar capital mínimo obligatorio
    if capital < 5:
    ...
    # No permitir compras de cantidad cero
    if qty <= 0:
...
# ======================================================
# MONITOREO (TP / SL DINÁMICO)
# ======================================================

def monitor_trade(position):
    """
    Monitorea una operación activa cada 2 segundos hasta
    alcanzar TP o SL.
    """
...
def trading_cycle(user_id):
    """
    1. Escanea el mercado CoinW
    2. Selecciona el mejor par con breakout agresivo
    3. Ejecuta compra
    4. Monitoreo continuo hasta TP o SL
    """
